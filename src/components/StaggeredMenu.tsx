...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // Get CSS variable values or use provided colors
  const getMenuButtonColor = () => {
    if (menuButtonColor) return menuButtonColor;
  ...
  const getOpenMenuButtonColor = () => {
    if (openMenuButtonColor) return openMenuButtonColor;
    if (typeof window !== "undefined") {
      return (
        getComputedStyle(document.documentElement)
  ...
  const resolvedMenuButtonColor = getMenuButtonColor();
  const resolvedOpenMenuButtonColor = getOpenMenuButtonColor();
  ...
  const animateIcon = useCallback(() => {
    // Icon animation now handled by CSS (hamburger to X)
    // Keeping this function for compatibility but no GSAP animation needed
    return;
  }, []);
